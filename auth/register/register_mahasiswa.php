...
<body>
    <div class="title">
        ...
        <p class="text-center mt-4 h6">Daftar Akun Mahasiswa</p>
        <p class="text-center">Sudah Punya Akun ? <a href="http://e-learning.test/auth/login/">Login Disini</a></p>
    </div>
    <div class="row g-3">
        <input type="text" name="mahasiswa" class="d-none">
        <div class="col-12">
            <label for="inputAddress" class="form-label">NRP</label>
            <input type="text" class="form-control" name="nrp" maxlength="10">
        </div>
        <div class="col-12">
        ...
        <div class="col-12">
            <label for="inputAddress2" class="form-label">Nomer Telepon</label>
            <input type="text" class="form-control" name="nomer_telp">
        </div>
        <div class="col-md-6">
        ...
        <div class="col-12 d-flex justify-content-center">
            <button class="btn btn-primary btn-daftar" name="daftar">Daftar</button>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $(".btn-daftar").click(function() {
                let nrp = $("input[name='nrp']").val();
                let nama = $("input[name='nama']").val();
                let nomer_telp = $("input[name='nomer_telp']").val();
                let username = $("input[name='username']").val();
                let password = $("input[name='password']").val();
                let mahasiswa = $("input[name='mahasiswa']").val();

                if (nrp.length == "") {
                    swal("Oops!", "NIP Wajib Diisi !", "error");
                } else if (nama.length == "") {
                    swal("Oops!", "Nama Lengkap Wajib Diisi !", "error");
                } else if (nomer_telp.length == "") {
                    swal("Oops!", "Nomer Telepon Wajib Diisi !", "error");
